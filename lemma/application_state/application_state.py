...
class ApplicationState():
    ...
    def update_implicit_x_position():
        ...
        if document == None:
            ...
            value = Layouter.get_absolute_xy(Layouter.get_node_layout(insert))[0]
    ...
    def scroll_insert_on_screen(document, animation_type='default'):
        window_height = ApplicationState.view_height
        insert_node = document.get_insert_node()
        insert_position = Layouter.get_absolute_xy(Layouter.get_node_layout(insert_node))

        content_offset = LayoutInfo.get_normal_document_offset()
        insert_y = insert_position[1] + content_offset
        insert_height = Layouter.get_node_layout(insert_node)['height']
        scrolling_offset_y = ApplicationState.get_current_scrolling_offsets()[1]
        content_height = Layouter.get_height() + LayoutInfo.get_document_padding_bottom() + LayoutInfo.get_normal_document_offset() + ApplicationState.title_buttons_height

        if window_height <= 0:
            new_position = (0, 0)
        elif Layouter.get_absolute_xy(Layouter.get_line_layout_at_y(insert_position[1]))[1] == 0:
            new_position = (0, 0)
        ...
        elif insert_position[1] >= Layouter.get_height() - insert_height and content_height >= window_height:
            new_position = (0, Layouter.get_height() + content_offset + LayoutInfo.get_document_padding_bottom() - window_height)
    ...
    def get_current_scrolling_offsets():
        ...
        time_since_last_scroll = ApplicationState.frame_time - ApplicationState.last_scroll_scheduled

        if ApplicationState.last_scroll_animation_type == 'default' and time_since_last_scroll < 200000:
            proximity = animation_factors_default[int(time_since_last_scroll * 0.000064)]
            x = ApplicationState.prev_scrolling_target_x + proximity * (ApplicationState.scrolling_target_x - ApplicationState.prev_scrolling_target_x)
            y = ApplicationState.prev_scrolling_target_y + proximity * (ApplicationState.scrolling_target_y - ApplicationState.prev_scrolling_target_y)

        elif ApplicationState.last_scroll_animation_type == 'decelerate' and time_since_last_scroll < 1800000:
        ...
        max_y = max(0, LayoutInfo.get_normal_document_offset() + ApplicationState.title_buttons_height + Layouter.get_height() + LayoutInfo.get_document_padding_bottom() - ApplicationState.view_height)
        max_x = max(0, LayoutInfo.get_document_padding_left() + Layouter.get_width() - ApplicationState.view_width)

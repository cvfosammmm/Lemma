...
class Toolbars():
    ...
    def __init__(self, main_window, application):
        ...
        self.toolbar.main_right.edit_menu_button.add_controller(self.edit_button_controller)
    ...
    def on_edit_button_press(self, controller=None, n_press=None, x=None, y=None):
        button = self.toolbar.main_right.edit_menu_button
        allocation = button.compute_bounds(self.main_window).out_bounds
        x = allocation.origin.x + allocation.size.width / 2
    ...
    def update(self):
        ...
        if mode == 'documents':
            ...
            widget = document.get_selected_widget()

            if widget != None and self.application.widget_manager.has_toolbar(widget):
                if self.toolbar.stack_left.get_child_by_name(widget.get_type()) == None:
                    self.toolbar.stack_left.add_named(self.application.widget_manager.get_toolbar(widget), widget.get_type())

                self.toolbar.stack_left.set_visible_child_name(widget.get_type())
                ...
                if edit_link_visible:
                    ...
                    self.toolbar.main_left.insert_link_button.set_tooltip_text(_('Insert Link') + ' (' + Shortcuts.get_for_labels('link_popover') + ')')
                self.toolbar.stack_right.set_visible_child_name('main')
    ...
    def update_button_visibility(self):
        ...
        self.toolbar.main_left.italic_button.set_visible(Settings.get_value('button_visible_italic'))
        self.toolbar.main_left.verbatim_button.set_visible(Settings.get_value('button_visible_verbatim'))
        self.toolbar.main_left.highlight_button.set_visible(Settings.get_value('button_visible_highlight'))
        self.toolbar.main_left.indent_less_button.set_visible(Settings.get_value('button_visible_decrease_indent'))
        self.toolbar.main_left.indent_more_button.set_visible(Settings.get_value('button_visible_increase_indent'))
    ...
    def update_tag_toggle(self, tagname):
        button_dict = {'bold': self.toolbar.main_left.bold_button, 'italic': self.toolbar.main_left.italic_button, 'verbatim': self.toolbar.main_left.verbatim_button, 'highlight': self.toolbar.main_left.highlight_button}

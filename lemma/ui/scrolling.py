...
class DocumentScrolling():
    ...
    def __init__(self, main_window, application):
        ...
        MessageBus.subscribe(self, 'cursor_movement')
    ...
    def animate(self):
        ...
        if 'cursor_movement' in messages or 'undo_executed' in messages or 'redo_executed' in messages or 'preedit_changed' in messages:
    ...
    def scroll_to_xy(self, x, y, animation_type='default'):
        ...
        self.set_target(x, y, animation_type)

    @timer.timer
    def scroll_insert_on_screen(self, animation_type='default'):
        document = WorkspaceRepo.get_workspace().get_active_document()
        if document == None: return
        ...
        if new_position[0] != self.target_x or new_position[1] != self.target_y:
            self.set_target(new_position[0], new_position[1], animation_type)

    @timer.timer
    def decelerate(self, x, y, vel_x, vel_y):
        document = WorkspaceRepo.get_workspace().get_active_document()
        if document == None: return
        ...
        self.set_target(x, y, 'decelerate')
